...
def pollEvieEW():
    ...
    cur = conn.cursor()
    cur.execute(f"SELECT meeting FROM meeting_pool WHERE date = '{datetime.now().strftime('%d/%m/%y')}' AND abandoned != 't'")
    ...
    cur.execute(f"SELECT race_id, track, time FROM all_races WHERE date = '{datetime.now().strftime('%d/%m/%y')}'")
    ...
    cur.execute(f"SELECT race_id, bookie, win_back, scrape_utctimestamp FROM oc_back_scrapes WHERE scrape_utctimestamp >= {orangeTimeStamp}")
    ...
    for i in allOCBackScrapes:
        if i[0] not in allOCBackScrapesDict:
            ...
            allOCBackScrapesDict[i[0]][i[1]] = {'winBack' : i[2], 'scrapeTimeStamp':i[3]}
            ...
            if raceID in allOCBackScrapesDict:
                ...
                for bookie in allBookies:
                    if bookie not in allOCBackScrapesDict[raceID]:
                        tempComponent['status'] = "Red"
                        tempComponent['info'] += f"\n{allRacesDict[raceID]} {bookie} odds missing"
                    else:
                #for bookie in allOCBackScrapesDict[raceID]:
                        if allOCBackScrapesDict[raceID][bookie]['winBack'] <= 1:
                                tempComponent['status'] = "Red"
                                tempComponent['info'] += f"{allRacesDict[raceID]} {bookie} odds <= 1"
                        elif allOCBackScrapesDict[raceID][bookie]['scrapeTimeStamp'] >= greenTimeStamp:
                            if tempComponent['status'] == "None":
                                tempComponent['status'] = "Green"
                        elif allOCBackScrapesDict[raceID][bookie]['scrapeTimeStamp'] >= orangeTimeStamp:
                            if tempComponent['status'] != "Red":
                                tempComponent['status'] = "Orange"
    ...
    #general meetings check
    #check all races have odds for all bookies that are not 0 or 1 in desired timeframes


    #exchanges check. same as 2ups
